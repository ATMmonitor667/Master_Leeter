...
export default function ConstellationGrid() {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      // `for…of` rather than an index: this repo compiles with
      // `noUncheckedIndexedAccess`, so `nodes[i]` is `Node | undefined` and every
      // field access below would be a type error.
      for (const n of nodes) {
        ...
        // Restoring force back to the home anchor point (baseX, baseY)
      ...
      // Draw connections.
      //
      // This is a full pairwise scan with an early distance reject, not spatial
      // partitioning — at the default 55px spacing a 1080p viewport is ~750
      // nodes, so ~280k comparisons every frame. See the note in the PR/summary
      // before putting this anywhere that shares a CPU with the audio worklet.
      ...
      for (const n of nodes) {
        ...
        // Spatial radar rings on active proximity
        if (dist < 90) {
    ...
    <div className="relative w-full h-screen overflow-hidden select-none bg-slate-950">
      ...
      <div className="relative z-10 flex h-full flex-col items-center justify-center text-center px-4 pointer-events-none mix-blend-difference text-white">
        ...
        <p className="mt-4 font-mono text-xs md:text-sm max-w-lg opacity-70">
          High-velocity dynamic mesh. Sweep your cursor quickly across the grid to unleash kinetic
          shockwaves.
        </p>
      </div>
    </div>
